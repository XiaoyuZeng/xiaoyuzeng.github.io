...
### 2.To identify those people with deontological and utilitarian preference by checking whether they behave consistently
...
```{r,setup,echo=FALSE,results=F,include=FALSE}
options(htmltools.dir.version = FALSE)
options(servr.daemon = TRUE)
```

```{r,xaringan-themer,echo=FALSE,results=F,include=FALSE}
library(xaringanthemer)
...
data$consistent_action_rate <- (data$acinc+data$accon+
                                   data$ininc+data$incon)/24
```
...
data$NOcluster_complete <- k2$cluster
#str(k2)
...
clusterone_complete <- filter(data,data$NOcluster_complete==1)
clustertwo_complete <- filter(data,data$NOcluster_complete==2)
clusterthree_complete <- filter(data,data$NOcluster_complete==3)
clusterfour_complete <- filter(data,data$NOcluster_complete==4)

clusterone_complete <- data.frame(clusterone_complete$ID,clusterone_complete$perc_acinc,clusterone_complete$perc_accon,clusterone_complete$consistent_action_rate)

clustertwo_complete <- data.frame(clustertwo_complete$ID,clustertwo_complete$perc_acinc,clustertwo_complete$perc_accon,clustertwo_complete$consistent_action_rate)

clusterthree_complete <- data.frame(clusterthree_complete$ID,clusterthree_complete$perc_acinc,clusterthree_complete$perc_accon,clusterthree_complete$consistent_action_rate)

clusterfour_complete <- data.frame(clusterfour_complete$ID,clusterfour_complete$perc_acinc,clusterfour_complete$perc_accon,clusterfour_complete$consistent_action_rate)

# mean(clusterone$perc_acinc_2)
# mean(clusterone$perc_accon)
# mean(clustertwo$perc_acinc_2)
# mean(clustertwo$perc_accon)
# mean(clusterthree$perc_acinc_2)
# mean(clusterthree$perc_accon)
# mean(clusterfour$perc_acinc_2)
# mean(clusterfour$perc_accon)
```
...
```{r echo=FALSE}
nrow(clusterone_complete)
mean(clusterone_complete$clusterone_complete.perc_acinc)
mean(clusterone_complete$clusterone_complete.perc_accon)
formattable(clusterone_complete,list(
  perc_acinc = color_bar('lightpink'),
  perc_accon = color_bar('lightblue'),
  consistent_action_rate=color_bar('blue')
))
...
### 
```{r echo=FALSE}
nrow(clustertwo_complete)
mean(clustertwo_complete$clustertwo_complete.perc_acinc) 
mean(clustertwo_complete$clustertwo_complete.perc_accon)
formattable(clustertwo_complete,list(
  perc_acinc = color_bar('lightpink'),
  perc_accon = color_bar('lightblue'),
  consistent_action_rate=color_bar('blue')
))
...
## Let's take a look at these groups
```{r echo=FALSE}
nrow(clusterthree_complete)
mean(clusterthree_complete$clusterthree_complete.perc_acinc)
mean(clusterthree_complete$clusterthree_complete.perc_accon)
formattable(clusterthree_complete,list(
  perc_acinc = color_bar('lightpink'),
  perc_accon = color_bar('lightblue'),
  consistent_action_rate=color_bar('blue')
))
...
## Let's take a look at these groups
```{r echo=FALSE}
nrow(clusterfour_complete)
mean(clusterfour_complete$clusterfour_complete.perc_acinc)
mean(clusterfour_complete$clusterfour_complete.perc_accon)
formattable(clusterfour_complete,list(
  perc_acinc = color_bar('lightpink'),
  perc_accon = color_bar('lightblue'),
  consistent_action_rate=color_bar('blue')
))
...
---
  ### People with **deontological** preference in moral dilemmas
  
```{r echo=FALSE}

deon_data <- filter(data,data$perc_acinc<=1/3&data$perc_accon<=1/3)
deon_data <- select(deon_data,ID,perc_acinc,perc_accon,consistent_action_rate)
nrow(deon_data)
mean(deon_data$perc_acinc)
mean(deon_data$perc_accon)
formattable(deon_data,list(
    perc_acinc = color_bar('lightpink'),
    perc_accon = color_bar('lightblue'),
    consistent_action_rate = color_bar('lightpink')
  ))
```

---
### People with **utilitarian** preference in moral dilemmas
  
```{r echo=FALSE}

utili_data <- filter(data,data$perc_acinc>=2/3&data$perc_accon<=1/3)
utili_data <- select(utili_data,ID,perc_acinc,perc_accon,consistent_action_rate)
nrow(utili_data)
mean(utili_data$perc_acinc)
mean(utili_data$perc_accon)
mean(utili_data$consistent_action_rate)
summary(utili_data$consistent_action_rate)
...
```
```{r}
write.table(deon_data,'clipboard',sep='\t',row.names=FALSE,col.names = FALSE)
write.table(utili_data,'clipboard',sep='\t',row.names=FALSE,col.names = FALSE)
```
...
kittyR::kittyR(meow = TRUE,sound = 3,only_kitten = TRUE)
```
...
#look at all choices using likert

##item

OUS_SUM_dt[,'data.A1_Q251']<- factor(OUS_SUM_dt[,'data.A1_Q251'])
OUS_SUM_dt[,'data.A2_Q251']<- factor(OUS_SUM_dt[,'data.A2_Q251'])
OUS_SUM_dt[,'data.A3_Q251']<- factor(OUS_SUM_dt[,'data.A3_Q251'])
OUS_SUM_dt[,'data.A4_Q251']<- factor(OUS_SUM_dt[,'data.A4_Q251'])
OUS_SUM_dt[,'data.A5_Q251']<- factor(OUS_SUM_dt[,'data.A5_Q251'])
OUS_SUM_dt[,'data.A6_Q251']<- factor(OUS_SUM_dt[,'data.A6_Q251'])
OUS_SUM_dt[,'data.A7_Q251']<- factor(OUS_SUM_dt[,'data.A7_Q251'])
OUS_SUM_dt[,'data.A8_Q251']<- factor(OUS_SUM_dt[,'data.A8_Q251'])
OUS_SUM_dt[,'data.A9_Q251']<- factor(OUS_SUM_dt[,'data.A9_Q251'])


names(OUS_SUM_dt) <-c(
  data.A1_Q251 ='在紧急情况下牺牲一个人自己的腿',
  data.A3_Q251 ='伤害这个人帮助另外好几个无辜人',
  data.A5_Q251 ='有义务将我们的一个肾捐给肾衰竭的人',
  data.A7_Q251 ='确保人们整体福祉和幸福,使用政治迫害，那么就应该使用政治迫害',
  data.A9_Q251 ='平等关心地球上所有人的福祉',
  data.A2_Q251 ='获取防止炸弹爆炸的有效信息，对一个无辜的人施以酷刑',
  data.A4_Q251 ='没能成功帮助别人和自己去主动伤害他人是一样错误的',
  data.A6_Q251 ='无辜的人们的死亡, 整体来看更多人被拯救。',
  data.A8_Q251 ='保留一个人并不真正需要的钱在道德上是错误的'
)


summary(OUS_SUM_dt)
##visualize survey using likert
#Here we will recode each factor and explicitly set the levels

for(i in seq_along(OUS_SUM_dt)) {
	OUS_SUM_dt[,i] <- factor(OUS_SUM_dt[,i], levels=1:7)
}


OUS_likert <- likert(OUS_SUM_dt)
